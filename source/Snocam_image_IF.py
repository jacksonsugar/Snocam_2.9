...
if len(os.listdir('{}/Snocam_pics'.format(configDir))) == 0:
...
def picture(configDir, NumSamples, RECOVER):
    try:
        ...
        if RECOVER == 0:
            ...
            camera.capture('{}/Snocam_data/INI/{}_Pic-INI.jpg'.format(configDir, samp_time))

        else:      

            samp_time = "{}-{}-{}".format(samp_count, NumSamples, samp_time)
            camera.capture('{}/Snocam_data/FIN/{}_Pic-FIN.jpg'.format(configDir, samp_time))
